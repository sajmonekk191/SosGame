...
namespace SOS_Essential.CustomControls
{
    public class SlideButton : Control
    {
        ...
        public SlideButton()
        {
            ...
            Cursor = Cursors.Hand;
            DoubleBuffered = true;
            artis = 4; //increment pro jezdící animaci
            diameter = 30;
            textEnabled = true;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (Enabled)
            {
                using (SolidBrush brush = new SolidBrush(isOn ? Color.Green : Color.LightGray))
                ...
                if (textEnabled)
                    using (Font font = new Font("Arial", 8.2f * diameter / 30, FontStyle.Bold))
                    {
                        ...
                        e.Graphics.DrawString(on, font, Brushes.Black, 5, y + 1);

                        height = TextRenderer.MeasureText(off, font).Height;
                        y = (diameter - height) / 2f;
                        e.Graphics.DrawString(off, font, Brushes.Black, diameter + 2, y + 1);
                    }
